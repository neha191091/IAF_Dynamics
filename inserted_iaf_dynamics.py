...
class BaselineTransitionNoKL(object):
    ...
    def __init__(self, n_latent, n_enc, n_control, n_secenc = 10):
        ...
        n_henc = n_secenc           #n_secenc indicates the number of dimensions for encoding [z, enc, u] for creating h for iaf
        ...
        #TODO: Check if the implementations for q_henc and q_iaf are correct

        self.q_iaf = AR_Net(z_size = z_size_q, h_size = n_henc, shape = layers)
        self.q_henc = Mlp(n_input_q, layers, n_henc, activation, out_activation)
    ...
    def q_transition_IAF(self, z, h):

        # Get mean and var
        var_f0, z_temp = self.q_iaf(h,z)
